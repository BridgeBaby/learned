...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx_h2, dw2, db2 = affine_backward(dx, out_cache)
        dx_h1, dw1, db1 = affine_relu_backward(dx_h2, h1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        def affine_bn_relu_forward(X, w, b, gamma, beta, bn_param):
            ...
            out, cache_re = relu_forward(bn)
            cache = (cache, cache_bn, cache_re)
            return out, cache
            
        def affine_bn_relu_backward(dout, cache):
            cache, cache_bn, cache_re = cache
            dx_re = relu_backward(dout, cache_re)
            dx_bn, dgamma, dbeta = batchnorm_backward_alt(dx_re, cache_bn)
            dx, dw, db = affine_backward(dx_bn, cache)
        ...
        for i in range(self.num_layers - 1):
            if self.normalization == 'batchnorm':
                out, cache_hid[i] = affine_bn_relu_forward(out, self.params['W%d'%(i+1)],
                ...
                out, cache_hid[i] = affine_relu_forward(out, w, b)
        ...
        dout, dw, db = affine_backward(dx, cache)
        grads['W%d'%(self.num_layers,)] = dw + self.reg * self.params['W%d'%(self.num_layers,)]
        grads['b%d'%(self.num_layers,)] = db
        for i in range(self.num_layers - 1):
            ri = self.num_layers - i -2
            loss += 0.5 * self.reg * np.sum(np.square(self.params['W%d'%(ri+1)]))
            if self.use_dropout:
                dout = dropout_backward(dout, cache_dp[ri]) 
            if self.normalization == 'batchnorm':
                dout, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, cache_hid[ri])
            else:
                dout, dw, db = affine_relu_backward(dout, cache_hid[ri])
