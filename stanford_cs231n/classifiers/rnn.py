...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        out_word, cache_out = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == "rnn":
            h, cache_hidden = rnn_forward(out_word, h0, Wx, Wh, b)
        else:
            h, cache_hidden = lstm_forward(out_word, h0, Wx, Wh, b)
        
        out_vo, cache_vo = temporal_affine_forward(h, W_vocab, b_vocab)
        ...
        if self.cell_type == 'rnn':        
            dx, dh0, dWx,dWh, db = rnn_backward(dx_affine, cache_hidden)
        else:
            dx,dh0,dWx,dWh,db = lstm_backward(dx_affine,cache_hidden)            
        grads['Wx'] = dWx
        grads['Wh'] = dWh
        grads['b'] = db
        
        dW_embed = word_embedding_backward(dx, cache_out)
    ...
    def sample(self, features, max_length=30):
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        N, D = features.shape
        out, cache_init = temporal_affine_forward(features.reshape(N, 1, D), W_proj, b_proj)
        N, T, H = out.shape
        h0 = out.reshape(N, H)
        h = h0
